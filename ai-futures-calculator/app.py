...
def plot_human_labor(fig, times, values, row, col):
    ...
    fig.add_trace(
        go.Scatter(x=times.tolist(), y=values.tolist(),
                  ...
                  customdata=format_years_vectorized(times),
                  hovertemplate='Year: %{customdata}<br>Human Labor: %{y:.3s}<extra></extra>'),
        row=row, col=col
    )
...
def plot_ai_labor(fig, times, values, row, col):
    ...
    fig.add_trace(
        go.Scatter(x=times.tolist(), y=values.tolist(),
                  ...
                  mode='lines+markers', marker=dict(size=4),
                  customdata=format_years_vectorized(times),
                  hovertemplate='Year: %{customdata}<br>Inference Compute: %{y:.3s}<extra></extra>'),
        row=row, col=col
    )
...
def plot_experiment_compute(fig, times, values, row, col):
    ...
    fig.add_trace(
        go.Scatter(x=times.tolist(), y=values.tolist(),
                  ...
                  mode='lines+markers', marker=dict(size=4),
                  customdata=format_years_vectorized(times),
                  hovertemplate='Year: %{customdata}<br>Experiment Compute: %{y:.3s}<extra></extra>'),
        row=row, col=col
    )
...
def plot_training_compute_growth_rate(fig, times, values, row, col):
    ...
    fig.add_trace(
        go.Scatter(x=times.tolist(), y=values.tolist(),
                  ...
                  mode='lines+markers', marker=dict(size=4),
                  customdata=format_years_vectorized(times),
                  hovertemplate='Year: %{customdata}<br>Training Compute: %{y:.3s}<extra></extra>'),
        row=row, col=col
    )
...
def plot_software_efficiency(fig, times, values, row, col):
    ...
    fig.add_trace(
        go.Scatter(x=times.tolist(), y=values.tolist(),
                  ...
                  line=dict(color='#ff7f0e', width=3),
                  customdata=format_years_vectorized(times),
                  mode='lines+markers', marker=dict(size=4),                  hovertemplate='Year: %{customdata}<br>Software Efficiency: %{y:.3s}<extra></extra>'),
        row=row, col=col
    )

def plot_labor_comparison(fig, time_series, row, col):
    """Plot human vs AI labor comparison"""
    times_arr = time_series['time']
    fig.add_trace(
        go.Scatter(x=times_arr.tolist(), y=time_series['L_HUMAN'].tolist(),
                  name='Human Labor',
                  line=dict(color='#ff7f0e', width=2),
                  customdata=format_years_vectorized(times_arr),
                  mode='lines+markers', marker=dict(size=4),                  hovertemplate='Year: %{customdata}<br>%{fullData.name}: %{y}<extra></extra>'),
        row=row, col=col
    )
    fig.add_trace(
        go.Scatter(x=times_arr.tolist(), y=time_series['inference_compute'].tolist(),
...
def plot_compute_comparison(fig, time_series, row, col):
    ...
    times_arr = time_series['time']
    fig.add_trace(
        go.Scatter(x=times_arr.tolist(), y=time_series['experiment_compute'].tolist(),
                  name='Experiment Compute',
    ...
    fig.add_trace(
        go.Scatter(x=times_arr.tolist(), y=time_series['training_compute_growth_rate'].tolist(),
                  name='Training Compute',
...
def plot_cumulative_progress(fig, times, progress, row, col):
    ...
    # Add training compute and vertical line for AC time if available
    ...
    if results and 'training_compute' in results:
        ...
        fig.add_trace(
            go.Scatter(x=times.tolist(), y=training_compute.tolist(), 
                      ...
                      customdata=format_years_vectorized(times),
                      mode='lines+markers', marker=dict(size=4),                      hovertemplate='Year: %{customdata}<br>Training Compute: %{y:.3s}<extra></extra>'),
            row=row, col=col
        )
        ...
        if aa_time >= times.min() and aa_time <= times.max():
            fig.add_trace(
                go.Scatter(x=[aa_time, aa_time], 
                          ...
                          mode='lines',
                          hovertemplate=f'AC Time: {format_decimal_year_to_month_year(aa_time)}<br>AC Progress: {sc_progress:.3f}<extra></extra>' if sc_progress else f'AC Time: {format_decimal_year_to_month_year(aa_time)}<extra></extra>'),
                row=row, col=col
            )
...
def plot_automation_fraction(fig, times, automation_fraction, row, col):
    ...
    # Add vertical line for AC time if available
    results = session_data.get('results')
    if results and results.get('aa_time') is not None:
        aa_time = results['aa_time']
        sc_progress = results.get('sc_progress_level')
...
def plot_rate_components(fig, times, progress_rates, training_compute_growth_rates, row, col):
    ...
    fig.add_trace(
        go.Scatter(x=times.tolist(), y=progress_rates.tolist(),
                  ...
                  customdata=format_years_vectorized(times),
                  mode='lines',                  hovertemplate='Year: %{customdata}<br>%{fullData.name}: %{y}<extra></extra>'),
        row=row, col=col
    )
    fig.add_trace(
...
def plot_ai_research_taste_vs_effective_compute(fig, effective_compute, ai_research_taste, row, col):
    """Plot AI research taste vs effective compute as a scatter plot"""
    fig.add_trace(
        go.Scatter(x=effective_compute.tolist(), y=ai_research_taste.tolist(),
                  name='AI Research Taste vs Effective Compute',
                  mode='markers',
                  marker=dict(color='#1f77b4', size=6, opacity=0.7)),
        row=row, col=col
    )

def plot_ai_research_taste_sd_vs_effective_compute(fig, effective_compute, ai_research_taste_sd, row, col):
    """Plot AI research taste SDs vs effective compute as a scatter plot"""
    fig.add_trace(
        go.Scatter(x=effective_compute.tolist(), y=ai_research_taste_sd.tolist(),
                  name='AI Research Taste (SD) vs Effective Compute',
                  mode='markers',
...
def plot_aggregate_research_taste_vs_effective_compute(fig, effective_compute, aggregate_research_taste, row, col):
    """Plot aggregate research taste vs effective compute as a scatter plot"""
    fig.add_trace(
        go.Scatter(x=effective_compute.tolist(), y=aggregate_research_taste.tolist(),
                  name='Aggregate Research Taste vs Effective Compute',
                  mode='markers',
                  marker=dict(color='#bcbd22', size=6, opacity=0.7)),
        row=row, col=col
    )
    # Add horizontal reference line at y=1 (no research taste enhancement)
    fig.add_hline(y=1.0, line_dash="dash", line_color="gray", opacity=0.5, row=row, col=col)

def plot_ai_research_taste_vs_ai_research_taste_sd(fig, ai_research_taste_sd, ai_research_taste, row, col):
    """Plot AI research taste vs AI research taste SDs as a scatter plot"""
    fig.add_trace(
        go.Scatter(x=ai_research_taste_sd.tolist(), y=ai_research_taste.tolist(),
                  name='AI Research Taste vs SD',
                  mode='markers',
                  marker=dict(color='#e377c2', size=6, opacity=0.7)),
        row=row, col=col
    )

def plot_aggregate_research_taste_vs_ai_research_taste_sd(fig, ai_research_taste_sd, aggregate_research_taste, row, col):
    """Plot aggregate research taste vs AI research taste SDs as a scatter plot"""
    fig.add_trace(
        go.Scatter(x=ai_research_taste_sd.tolist(), y=aggregate_research_taste.tolist(),
                  name='Aggregate Research Taste vs AI SD',
                  mode='markers',
        ...
        if sc_time >= times.min() and sc_time <= times.max():
            fig.add_trace(
                go.Scatter(x=[sc_time, sc_time], 
                          ...
                          line=dict(color='#d62728', width=2, dash='dash'),
                          mode='lines',
                          hovertemplate=f'AC Time: {format_decimal_year_to_month_year(sc_time)}<br>AC Progress: {sc_progress:.3f}<extra></extra>' if sc_progress else f'AC Time: {format_decimal_year_to_month_year(sc_time)}<extra></extra>'),
                row=row, col=col
            )
...
def plot_capability_eci(fig, times, capability_ecis, row, col, eci_data=None):
    """Plot capability ECI score over time with real-world ECI data points"""
    # Plot model trajectory
    fig.add_trace(
        go.Scatter(x=times.tolist(), y=capability_ecis.tolist(),
                  name='Model Prediction',
                  line=dict(color='#9467bd', width=3),
                  customdata=format_years_vectorized(times),
                  mode='lines+markers', marker=dict(size=4, color='#9467bd'),                  hovertemplate='Year: %{customdata}<br>%{fullData.name}: %{y:.1f}<extra></extra>'),
        row=row, col=col
    )

    # Add ECI data points if available
    if eci_data is not None and len(eci_data) > 0:
        # Extract data
        eci_times = [p['decimal_year'] for p in eci_data]
        eci_scores = [p['eci_score'] for p in eci_data]
        eci_labels = [p['display_name'] for p in eci_data]
        eci_orgs = [p['organization'] for p in eci_data]

        # Create color mapping for different organizations
        org_colors = {
            'OpenAI': '#10A37F',           # OpenAI green
            'Anthropic': '#832000',        # Dark red/brown for Anthropic
            'Google DeepMind': '#4285F4',  # Google blue
            'xAI': '#000000',              # Black for xAI
            'Alibaba': '#FF6A00',          # Alibaba orange
            'DeepSeek': '#0066CC',         # Deep blue for DeepSeek
            'Mistral AI': '#F2A900',       # Yellow/gold for Mistral
            'default': '#215F9A'           # Default blue
        }

        # Create symbol mapping for different organizations
        org_symbols = {
            'OpenAI': 'circle',
            'Anthropic': 'diamond',
            'Google DeepMind': 'square',
            'xAI': 'x',
            'Alibaba': 'triangle-up',
            'DeepSeek': 'star',
            'Mistral AI': 'hexagon',
        ...
        # Plot each point individually to allow custom colors and symbols
        for i, (time, score, label, org) in enumerate(zip(eci_times, eci_scores, eci_labels, eci_orgs)):
            # Determine organization color and symbol
            org_color = org_colors.get(org, org_colors['default'])
            org_symbol = org_symbols.get(org, org_symbols['default'])

            # Format organization for multi-country orgs
            org_display = org.split(',')[0] if ',' in org else org
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    customdata=format_years_vectorized(times),
                    text=[label],                    hovertemplate='<b>%{customdata}</b><br>%{customdata[2]}<br>Year: %{y:.3s}<br>ECI Score: %{y:.1f}<extra></extra>',
                    showlegend=True
                ),
                row=row, col=col
            )
    ...
    if results and results.get('sc_time') is not None:
        ...
        sc_progress = results.get('sc_progress_level')
        if sc_time >= times.min() and sc_time <= times.max():
            # Determine y-axis range based on both model and data
            y_min = capability_ecis.min()
            y_max = capability_ecis.max()
            if eci_data and len(eci_data) > 0:
                eci_scores_array = np.array([p['eci_score'] for p in eci_data])
                y_min = min(y_min, eci_scores_array.min())
                y_max = max(y_max, eci_scores_array.max())
...
def format_years_vectorized(times):
    """Vectorized conversion of decimal years to 'Mon YYYY' format for hover text.

    This is much faster than list comprehensions calling format_decimal_year_to_month_year.
    Uses numpy operations where possible and batches the formatting.

    Args:
        times: numpy array of decimal years

    Returns:
        list of formatted strings like 'Jan 2025'
    """
    ...
    year_fractions = times - years
    month_indices = np.round(year_fractions * 12).astype(int)

    # Handle edge case where rounding gives 12
    overflow = month_indices >= 12
    month_indices[overflow] = 0
    years[overflow] += 1

    # Batch format (still need loop for string assembly, but minimized)
    return [f"{month_abbrevs[m]} {y}" for m, y in zip(month_indices, years)]
...
def plot_horizon_lengths(fig, times, horizon_lengths, row, col, metr_data=None, ac_time_horizon_minutes=None):
    ...
    max_horizon = PARAMETER_BOUNDS['ac_time_horizon_minutes'][1]  # 1 million minutes
    ...
    # Format both years and horizon durations for hover text
    years_formatted = format_years_vectorized(times)
    horizons_formatted = [format_time_duration(h) for h in capped_horizon_lengths]
    fig.add_trace(
        go.Scatter(x=times.tolist(), y=capped_horizon_lengths.tolist(),
                  name='Model Prediction',
                  line=dict(color='#1f77b4', width=3),
                  mode='lines+markers', marker=dict(size=4, color='#1f77b4'),
                  customdata=list(zip(years_formatted, horizons_formatted)),
                  hovertemplate='Year: %{customdata[0]}<br>Horizon: %{customdata[1]}<extra></extra>'),
        row=row, col=col
    )
    ...
    if metr_data is not None and len(metr_data) > 0:
        ...
        if sota_p80_points:
            ...
            for i, (time, horizon, label, formatted) in enumerate(zip(sota_p80_times, sota_p80, sota_p80_labels, sota_p80_formatted)):
                ...
                # Format year for this single point
                year_formatted = format_decimal_year_to_month_year(time)
                fig.add_trace(
                    go.Scatter(x=[time], y=[horizon],
                              ...
                              customdata=[[year_formatted, formatted]],
                              text=[label],                              hovertemplate='<b>%{text}</b><br>Year: %{customdata[0]}<br>p80 Horizon: %{customdata[1]}<extra></extra>',
                              showlegend=True),
                    row=row, col=col
                )
    
    # Add horizontal dashed line for superhuman coder time horizon
    if ac_time_horizon_minutes is not None:
        sc_formatted = format_time_duration(ac_time_horizon_minutes)
        fig.add_trace(
            go.Scatter(x=[times.min(), times.max()], 
                      y=[ac_time_horizon_minutes, ac_time_horizon_minutes],
                      name='AC Time Horizon',
                      line=dict(color='#d62728', width=2, dash='dash'),
                      mode='lines',
                      hovertemplate=f'AC Time Horizon: {sc_formatted}<extra></extra>'),
            row=row, col=col
        )
...
def plot_horizon_lengths_vs_progress(fig, progress_values, horizon_lengths, row, col, metr_data=None, ac_time_horizon_minutes=None, progress_at_aa=None):
    ...
    # Plot the model trajectory with capped values
    # Format horizon durations for hover text
    horizon_formatted = [format_time_duration(h) for h in capped_horizon_lengths]
    fig.add_trace(
        go.Scatter(x=progress_values.tolist(), y=capped_horizon_lengths.tolist(),
                  name='Model Prediction',
                  line=dict(color='#1f77b4', width=3),
                  mode='lines+markers', marker=dict(size=4, color='#1f77b4'),
                  customdata=horizon_formatted,
            ...
            for i, (progress, horizon, label, formatted) in enumerate(zip(sota_p80_progress, sota_p80, sota_p80_labels, sota_p80_formatted)):
                ...
                fig.add_trace(
                    go.Scatter(x=[progress], y=[horizon],
                              ...
                              hovertemplate='<b>%{text}</b><br>Progress: %{x:.3f}<br>p80 Horizon: %{customdata[0]}<extra></extra>',
                              showlegend=True),
                    row=row, col=col
                )
    ...
    if progress_at_aa is not None:
        fig.add_trace(
            go.Scatter(x=[progress_at_aa, progress_at_aa], 
                      ...
                      line=dict(color='#ff7f0e', width=2, dash='dash'),
                      mode='lines',
                      hovertemplate=f'Progress at AC: {progress_at_aa:.1f}<extra></extra>'),
            row=row, col=col
        )
...
def get_tab_configurations():
    ...
    plot_function_map = {
        'plot_horizon_lengths': lambda fig, data, r, c: plot_horizon_lengths(fig, data['metrics']['times'], data['metrics']['horizon_lengths'], r, c, data.get('metr_data'), data.get('parameters', {}).get('ac_time_horizon_minutes')),
        'plot_horizon_lengths_vs_progress': lambda fig, data, r, c: plot_horizon_lengths_vs_progress(fig, data['metrics']['progress'], data['metrics']['horizon_lengths'], r, c, data.get('metr_data'), data.get('parameters', {}).get('ac_time_horizon_minutes'), data.get('progress_at_aa')),
        ...
        'plot_ai_research_taste_vs_effective_compute': lambda fig, data, r, c: plot_ai_research_taste_vs_effective_compute(fig, data['metrics']['effective_compute'], data['metrics']['ai_research_taste'], r, c),
        'plot_ai_research_taste_sd_vs_effective_compute': lambda fig, data, r, c: plot_ai_research_taste_sd_vs_effective_compute(fig, data['metrics']['effective_compute'], data['metrics']['ai_research_taste_sd'], r, c),
        'plot_aggregate_research_taste_vs_effective_compute': lambda fig, data, r, c: plot_aggregate_research_taste_vs_effective_compute(fig, data['metrics']['effective_compute'], data['metrics']['aggregate_research_taste'], r, c),
        'plot_ai_research_taste_vs_ai_research_taste_sd': lambda fig, data, r, c: plot_ai_research_taste_vs_ai_research_taste_sd(fig, data['metrics']['ai_research_taste_sd'], data['metrics']['ai_research_taste'], r, c),
        'plot_aggregate_research_taste_vs_ai_research_taste_sd': lambda fig, data, r, c: plot_aggregate_research_taste_vs_ai_research_taste_sd(fig, data['metrics']['ai_research_taste_sd'], data['metrics']['aggregate_research_taste'], r, c),
        ...
        'plot_capability_eci': lambda fig, data, r, c: plot_capability_eci(fig, data['metrics']['times'], data['metrics']['capability_ecis'], r, c, data.get('eci_data')),
    }
...
def create_multi_tab_dashboard(metrics: Dict[str, Any], time_range: List[float] = None) -> Dict[str, go.Figure]:
    ...
    _fn_start = time_module.perf_counter()
    _profile = {}
    ...
    _profile['validate_and_clean'] = time_module.perf_counter() - _t0
    ...
    _profile['special_handling'] = time_module.perf_counter() - _t0

    # Process METR data for plotting
    _t0 = time_module.perf_counter()
    metr_data = process_metr_data()
    _profile['process_metr'] = time_module.perf_counter() - _t0

    # Process ECI data for plotting
    _t0 = time_module.perf_counter()
    eci_data = process_eci_data()
    _profile['process_eci'] = time_module.perf_counter() - _t0

    # Prepare data for plotting
    _t0 = time_module.perf_counter()
    _t_prep_start = _t0

    _t_params_start = time_module.perf_counter()
    params_dict = params_to_dict(session_data.get('current_params', Parameters())) if session_data.get('current_params') else {}
    _profile['params_to_dict'] = time_module.perf_counter() - _t_params_start

    _t0 = time_module.perf_counter()
    _time_series = session_data['time_series']
    _profile['  get_time_series'] = time_module.perf_counter() - _t0
    ...
    _profile['  build_dict'] = time_module.perf_counter() - _t0
    _profile['prepare_plot_data'] = time_module.perf_counter() - _t_prep_start

    # Get tab configurations
    _t0 = time_module.perf_counter()
    tab_configs = get_tab_configurations()
    _profile['get_tab_configs'] = time_module.perf_counter() - _t0
    ...
    for tab_config in tab_configs:
        try:
            _tab_start = time_module.perf_counter()
            figures[tab_config.tab_id] = create_tab_figure(tab_config, plot_data)
            tab_timings[tab_config.tab_id] = time_module.perf_counter() - _tab_start
        except Exception as e:
            logger.error(f"Failed to create tab '{tab_config.tab_name}': {e}")
            continue
    _profile['create_all_tabs'] = time_module.perf_counter() - _t0

    # Log profiling information
    _total_time = time_module.perf_counter() - _fn_start
    logger.info("=== create_multi_tab_dashboard() Profiling ===")
    logger.info(f"  Total time: {_total_time:.3f}s")
    for stage, t in sorted(_profile.items(), key=lambda x: -x[1]):
        _pct = 100.0 * t / _total_time if _total_time > 0 else 0.0
        logger.info(f"  {stage:25s}: {t:7.3f}s ({_pct:5.1f}%)")

    # Log individual tab timings
    if tab_timings:
        logger.info("  Tab creation breakdown:")
        for tab_id, t in sorted(tab_timings.items(), key=lambda x: -x[1]):
            _pct = 100.0 * t / _total_time if _total_time > 0 else 0.0
            logger.info(f"    {tab_id:23s}: {t:7.3f}s ({_pct:5.1f}%)")
    logger.info("=" * 45)
...
def compute_model():
    ...
    _t_get_dict = time_module.perf_counter() - _t0

    _t0 = time_module.perf_counter()
    params = Parameters(**params_dict)
    _t_create_params = time_module.perf_counter() - _t0

    _profile['parse_params'] = _t_get_dict + _t_create_params
    _profile['  get_params_dict'] = _t_get_dict
    _profile['  create_Parameters'] = _t_create_params
    ...
    _profile['time_series'] = time_module.perf_counter() - _t0
    ...
    _profile['init_model'] = time_module.perf_counter() - _t0

    _t0 = time_module.perf_counter()
    times, progress_values, research_stock_values = model.compute_progress_trajectory(
            time_range, initial_progress
        )
    _profile['compute_trajectory'] = time_module.perf_counter() - _t0
    ...
    _profile['serialize_plots'] = time_module.perf_counter() - _t0
    ...
    _profile['build_summary'] = time_module.perf_counter() - _t0

    # Log profiling information
    _total_time = time_module.perf_counter() - _fn_start
    logger.info("=== compute_model() Profiling ===")
    logger.info(f"  Total time: {_total_time:.3f}s")
    for stage, t in sorted(_profile.items(), key=lambda x: -x[1]):
        _pct = 100.0 * t / _total_time if _total_time > 0 else 0.0
        logger.info(f"  {stage:25s}: {t:7.3f}s ({_pct:5.1f}%)")
...
def export_csv():
    ...
    try:
        ...
        # Write header
        writer.writerow(['time', 'cumulative_progress', 'automation_fraction', 'progress_rate', 'software_progress_rate', 'coding_labor', 'research_stock', 'research_effort', 'experiment_capacity', 'human_only_progress_rate', 'ai_labor_contribution', 'human_labor_contribution', 'ai_software_progress_multiplier'])
        
        # Write metadata header with SC information if available
        if results.get('aa_time') is not None:
            writer.writerow(['# AC Level Reached at:', f"Time: {results['aa_time']:.4f}", f"Progress Level: {results.get('sc_progress_level', 'N/A'):.4f}" if results.get('sc_progress_level') is not None else 'Progress Level: N/A'])
        ...
        for i in range(len(results['times'])):
            ...
            # Add AI software progress multiplier (software uplift) if available
            if 'ai_sw_progress_mult_ref_present_day' in results and i < len(results['ai_sw_progress_mult_ref_present_day']):
                row.append(results['ai_sw_progress_mult_ref_present_day'][i])
            else:
                row.append(0.0)
...
def process_eci_data():
    """Process ECI data from epoch_capabilities_index.csv and return structured data for plotting"""
    try:
        # Load ECI data from CSV
        try:
            with open('epoch_capabilities_index.csv', 'r') as f:
                reader = csv.DictReader(f)
                eci_data = list(reader)
        except FileNotFoundError:
            logger.warning('epoch_capabilities_index.csv file not found')
            return None
        except Exception as e:
            logger.warning(f'Error reading epoch_capabilities_index.csv: {str(e)}')
            return None

        if session_data['results'] is None:
            logger.warning('No model results available for ECI data processing')
            return None
        ...
        # Calculate progress offset (same as METR data processing)
        start_year = times[0]
        target_initial_progress = progress_values[0]
        progress_at_start = np.interp(start_year, times, progress_values)
        progress_offset = target_initial_progress - progress_at_start
        adjusted_progress_values = progress_values + progress_offset

        # Process ECI data points
        eci_points = []

        for row in eci_data:
            # Skip rows without ECI score
            if not row.get('ECI Score') or row['ECI Score'].strip() == '':
                continue

            try:
                eci_score = float(row['ECI Score'])
            ...
            try:
                ...
                logger.warning(f"Could not parse release date for {row.get('Model name', 'unknown')}: {release_date_str} ({e})")
                continue

            # Interpolate progress value at the release date
            if decimal_year >= times.min() and decimal_year <= times.max():
                interpolated_progress = np.interp(decimal_year, times, adjusted_progress_values)
            elif decimal_year < times.min():
                interpolated_progress = adjusted_progress_values[0]
            ...
            point = {
                ...
                'display_name': row.get('Display name', row.get('Model name', 'Unknown')),
                'release_date': release_date_str,
                'decimal_year': decimal_year,
                'eci_score': eci_score,
                'interpolated_progress': interpolated_progress,
            ...
            eci_points.append(point)

        return eci_points

    except Exception as e:
        logger.warning(f"Error processing ECI data: {e}")
        return None
