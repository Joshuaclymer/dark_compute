...
def _gauss_hermite_expectation(func, mu: float, sigma: float) -> float:
    """
...
class AutomationModel:
    ...
    def get_progress_from_index(self, index:float) -> float:
        ...
        def get_prog_exponential(index:float) -> float:
            """Get the progress from the index"""
            assert False, "Exponential schedule type not implemented"
        def get_prog_logistic(index:float) -> float:
            """Get the progress from the index for logistic schedule.
            Inverse of: automation = L / (1 + exp(-k(progress - x0)))
            Solving: progress = x0 - (1/k) * ln(L/automation - 1)
            """
            # Clamp index to valid range for logistic inverse
            index = np.clip(index, 1e-9, self.logistic_L - 1e-9)
            if abs(self.logistic_k) < 1e-12:
                # k ~ 0, use midpoint
                return self.logistic_x0
            ratio = self.logistic_L / index - 1
            if ratio <= 0:
                # index >= L, return large progress value
                return self.logistic_x0 + cfg.SIGMOID_EXPONENT_CLAMP / abs(self.logistic_k)
            return self.logistic_x0 - np.log(ratio) / self.logistic_k

        if self.schedule_type == "linear":
            return get_prog_linear(index)
        elif self.schedule_type == "logistic":
            return get_prog_logistic(index)
        elif self.schedule_type == "exponential":
...
def compute_research_effort(experiment_compute: float, serial_coding_labor: float, alpha_experiment_capacity: float, rho: float, experiment_compute_exponent: float, aggregate_research_taste: float = cfg.AGGREGATE_RESEARCH_TASTE_BASELINE) -> float:
    ...
    if experiment_compute_exponent < cfg.experiment_compute_exponent_CLIP_MIN or experiment_compute_exponent > cfg.experiment_compute_exponent_CLIP_MAX:
        # Use debug level since this is expected with Monte Carlo sampling - the clamping is intentional
        logger.debug(f"experiment_compute_exponent value {experiment_compute_exponent} outside valid range, clamping to [{cfg.experiment_compute_exponent_CLIP_MIN}, {cfg.experiment_compute_exponent_CLIP_MAX}]")
...
class ProgressModel:
    ...
    def compute_milestones(self):
        ...
        if use_sar_baseline:
            ...
            print(f"\n=== TED-AI Baseline Adjustment ===")
            print(f"SAR experiment selection would be achieved at {sar_exp_selection_time_unconstrained:.2f}")
            print(f"AC achieved at {ac_time:.2f}")
            print(f"Using SD at AC time ({sd_at_ac:.2f}) as baseline for TED-AI instead of TOP_RESEARCHER_SD ({cfg.TOP_RESEARCHER_SD:.2f})")
            print(f"===================================\n")
        else:
        ...
        milestones = {
            ...
            'TED-AI': {
                'metric': 'ai_research_taste_sd',
                'target': ted_ai_baseline_sd * (1.0 + self.params.ted_ai_m2b),  # Linearly spaced in SD from baseline
                'interpolation_type': 'exponential'
            },
            'ASI': {
                'metric': 'ai_research_taste_sd',
                'target': ted_ai_baseline_sd * (1.0 + self.params.ted_ai_m2b + 2.0),  # 2 M2Bs above TED-AI
                'interpolation_type': 'exponential'
            },
        ...
        # Debug: Print SD progression info
        if 'ai_research_taste_sd' in self.results:
            sd_values = np.asarray(self.results['ai_research_taste_sd'], dtype=float)
            times = np.asarray(self.results['times'], dtype=float)
            max_sd = np.max(sd_values)
            max_sd_idx = np.argmax(sd_values)
            max_sd_time = times[max_sd_idx]
            print(f"\n=== AI Research Taste SD Progression ===")
            print(f"Max SD reached: {max_sd:.2f} at time {max_sd_time:.2f}")
            print(f"Simulation end time: {times[-1]:.2f}")
            print(f"TED-AI target: {cfg.TOP_RESEARCHER_SD * (1.0 + self.params.ted_ai_m2b):.2f} SD")
            print(f"ASI target: {cfg.TOP_RESEARCHER_SD * (1.0 + self.params.ted_ai_m2b + 2.0):.2f} SD")
            print(f"TED-AI achievable: {max_sd >= cfg.TOP_RESEARCHER_SD * (1.0 + self.params.ted_ai_m2b)}")
            print(f"ASI achievable: {max_sd >= cfg.TOP_RESEARCHER_SD * (1.0 + self.params.ted_ai_m2b + 2.0)}")
            print(f"========================================\n")
        ...
        # Print milestone achievement dates to console
        print("\n=== MILESTONE ACHIEVEMENT DATES ===")
        for name, milestone in milestones_list:
            print(f"{name}: {milestone['time']:.2f}")
        print("===================================\n")
