...
tool_types = ['DUV_scanner', 'Photoresist', 'Overlay_metrology', 'Ion_implanter', 'Electroplating', 'deposition']
tool_display_names = {
...
# Use World data
world_data = capabilities['world']

def get_status(tool, node, year):
    """Determine if a tool is None, Pilot, or Production for a given node and year"""
    pilot_date = world_data.get('pilot', {}).get(node, {}).get(tool)
    hvm_date = world_data.get('hvm', {}).get(node, {}).get(tool)
...
# Print text version
print("\nWorld Semiconductor Tool Capability Timeline (2005-2025)")
print("=" * 150)
...
plt.tight_layout()
plt.savefig('world_timeline_table.png', dpi=300, bbox_inches='tight')
print("Color-coded timeline table saved to world_timeline_table.png")
