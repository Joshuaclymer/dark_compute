...
class PRCBlackProjectStock():
    ...
    def add_black_project(self, year : float, compute_to_add):
        ...
        """
        if year not in self.black_project_added_per_year:
            self.black_project_added_per_year[year] = {}

        # Handle both Compute objects and scalar values
        # Use duck typing to check for Compute (handles cross-module imports)
        if hasattr(compute_to_add, 'chip_counts'):
            # Extract chip characteristics from the Compute object
            chip_id_counter = len(self.black_project_added_per_year[year])
    ...
    def black_project_dead_and_alive(self, year : float):
        ...
        for y in self.black_project_added_per_year:
            if y <= year:
                for chip_id, chip_data in self.black_project_added_per_year[y].items():
