...
from black_project_backend.black_project_parameters import ProcessNode, ProcessNodeStrategy, BlackFabParameters, BlackProjectParameters
...
Instantiate PRCBlackFab with construction parameters:
    fab = PRCBlackFab(
...
def set_localization_probabilities(black_fab_params):
    """
    Update the global PROBABILITY_OF_90P_PRC_LOCALIZATION_AT_NODE from BlackFabParameters.
    This should be called before creating any BlackFab instances to ensure they use the correct parameters.
    """
    global PROBABILITY_OF_90P_PRC_LOCALIZATION_AT_NODE
    PROBABILITY_OF_90P_PRC_LOCALIZATION_AT_NODE = black_fab_params.get_probability_of_90p_prc_localization_at_node()
...
def estimate_wafer_starts_per_month(
    ...
    median_labor_capacity = BlackFabParameters.wafers_per_month_per_worker * operation_labor
    ...
    sigma_relative = BlackFabParameters.labor_productivity_relative_sigma
    ...
    median_scanner_capacity = BlackFabParameters.wafers_per_month_per_lithography_scanner * number_of_prc_lithography_scanners_devoted_to_fab

    # Sample scanner capacity from a log-normal distribution (ESTIMATED: 20% relative uncertainty)
    sigma_relative_scanner = BlackFabParameters.scanner_productivity_relative_sigma
...
def estimate_construction_duration(
    ...
    def _construction_duration_given_wafer_capacity(wafer_starts_per_month: float) -> float:
        ...
        capacity1 = 5000
        time1 = BlackFabParameters.construction_time_for_5k_wafers_per_month
        capacity2 = 100000
        time2 = BlackFabParameters.construction_time_for_100k_wafers_per_month
        ...
        sigma_relative = BlackFabParameters.construction_time_relative_sigma
    ...
    construction_labor_requirement_given_wafer_capacity = (
        BlackFabParameters.construction_workers_per_1000_wafers_per_month / 1000
...
def estimate_transistor_density_relative_to_h100(process_node_nm: float) -> float:
    ...
    ) ** BlackFabParameters.transistor_density_scaling_exponent

    return density_relative_to_h100

def estimate_architecture_efficiency_relative_to_h100(year: float, architecture_efficiency_improvement_per_year: float, agreement_year: float = None) -> float:
    """
    ...
    Args:
        ...
        architecture_efficiency_improvement_per_year: Annual improvement factor (e.g., 1.23)
    ...
    efficiency_relative_to_h100 = architecture_efficiency_improvement_per_year ** years_since_h100
...
def estimate_total_prc_lithography_scanners_for_node(current_year: float, localization_year: float) -> float:
    ...
    median_total_scanners = (BlackFabParameters.prc_lithography_scanners_produced_in_first_year * (n + 1) +
                          BlackFabParameters.prc_additional_lithography_scanners_produced_per_year * n * (n + 1) / 2)

    # Add uncertainty to PRC scanner production (ESTIMATED: 30% relative uncertainty)
    sigma_relative = BlackFabParameters.prc_scanner_production_relative_sigma
...
def sample_us_estimate_of_prc_scanners(actual_prc_scanners: float, median_absolute_relative_error: float) -> float:
    ...
    """
    return sample_us_estimate_with_error(actual_prc_scanners, median_absolute_relative_error)


def lr_from_inventory_accounting(
    reported_prc_scanners: float,
    us_estimate_of_prc_scanners: float,
    optimal_diversion_proportion: float,
    ...
    Args:
        ...
        optimal_diversion_proportion: Proportion of scanners diverted to fab
        median_absolute_relative_error: Median absolute relative error of US intelligence estimate

    Returns:
        float: Likelihood ratio LR = P(evidence|fab exists) / P(evidence|fab doesn't exist)
    """
    # Handle edge case
    if reported_prc_scanners < 1e-10:
    ...
    return lr_from_discrepancy_in_us_estimate(
        true_if_project_exists=true_number_of_prc_scanners_if_fab_exists,
        true_if_no_project=reported_prc_scanners,
        us_estimate=us_estimate_of_prc_scanners,
        median_error=median_absolute_relative_error
    )
...
def predict_watts_per_tpp_from_transistor_density(transistor_density_m_per_mm2: float) -> float:
    ...
    """
    # Get parameters from BlackFabParameters
    params = BlackFabParameters
...
class PRCBlackFab(BlackFab):
    ...
    def __init__(
        ...
        self.architecture_efficiency_improvement_per_year = project_parameters.exogenous_trends.architecture_efficiency_improvement_per_year
        ...
        self.h100_sized_chips_per_wafer = BlackFabParameters.h100_sized_chips_per_wafer
        ...
        self.us_estimate_of_prc_lithography_scanners = sample_us_estimate_of_prc_scanners(
            ...
            project_parameters.detection_parameters.us_intelligence_median_error_in_estimate_of_prc_sme_stock
        )
        ...
        self.lr_inventory = lr_from_inventory_accounting(
            ...
            median_absolute_relative_error=project_parameters.detection_parameters.us_intelligence_median_error_in_estimate_of_prc_sme_stock
        )
    ...
    def get_monthly_production_rate(self, year):
        ...
        chip_architecture_efficiency_relative_to_h100 = estimate_architecture_efficiency_relative_to_h100(
            self.construction_start_year,
            self.architecture_efficiency_improvement_per_year,
            self.agreement_year
        )
