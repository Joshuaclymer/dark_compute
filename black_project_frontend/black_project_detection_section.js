...
function plotDarkComputeDetectionSection(data) {
    ...
    if (data.black_project_model) {
        ...
        function plotLRComponent(elementId, lrData, title, color = COLOR_PALETTE.detection, yRange = null) {
            if (!lrData) return;
            ...
            const traces = [
                ...
                },
                {
                    ...
                    mode: 'lines',
                    fill: 'tonexty',
                    fillcolor: COLOR_PALETTE.hexToRgba(color, 0.2),
                    line: { color: 'transparent' },
                    showlegend: false,
                    hoverinfo: 'skip'
                },
            ...
            if (yRange) {
                yaxisConfig.range = [Math.log10(yRange[0]), Math.log10(yRange[1])];
            }
        ...
        // Use colors from COLOR_PALETTE: chip_stock for PRC chip stock, fab for SME stock, datacenters_and_energy for satellite
        if (data.black_project_model.lr_prc_accounting && data.black_project_model.lr_prc_accounting.individual) {
            ...
            plotPDF('lrPrcAccountingPlot', lrPrcSamples, COLOR_PALETTE.chip_stock, 'Likelihood Ratio', 12, true, 1/3, 5);
        }
        if (data.black_project_model.lr_sme_inventory && data.black_project_model.lr_sme_inventory.individual) {
            ...
            plotPDF('lrSmeInventoryPlot', lrSmeInventorySamples, COLOR_PALETTE.fab, 'Likelihood Ratio', 12, true, 1/3, 5);
        }
        if (data.black_project_model.lr_satellite_datacenter && data.black_project_model.lr_satellite_datacenter.individual) {
            const lrSatelliteDatacenterSamples = data.black_project_model.lr_satellite_datacenter.individual;
            plotPDF('lrSatelliteDatacenterPlot', lrSatelliteDatacenterSamples, COLOR_PALETTE.datacenters_and_energy, 'Likelihood Ratio', 12, true, 1/3, 5);
        }
        // Plot reported energy consumption LR over time (varies over time as covert datacenters grow)
        if (data.black_project_model.lr_reported_energy) {
            plotLRComponent('lrReportedEnergyPlot', data.black_project_model.lr_reported_energy, 'Energy Consumption', COLOR_PALETTE.darken('datacenters_and_energy', 0.8), [0.5, 2]);
        }

        // Plot combined evidence from reported assets over time (includes energy which varies)
        if (data.black_project_model.lr_combined_reported_assets) {
            plotLRComponent('lrReportedAssetsInitialPlot', data.black_project_model.lr_combined_reported_assets, 'Combined Reported Assets');
            plotLRComponent('lrReportedAssetsPlot', data.black_project_model.lr_combined_reported_assets, 'Combined Reported Assets');
        }
...
function createDetectionLatencyPlot(elementId = 'detectionLatencyPlot') {
    ...
    const colors = {
        line: COLOR_PALETTE.detection,
        fill: COLOR_PALETTE.rgba('detection', 0.2),
        points: COLOR_PALETTE.detection
    };
...
function createIntelligenceAccuracyPlot(elementId = 'intelligenceAccuracyPlot') {
  ...
  const leftTraces = [];
  const pointColor = COLOR_PALETTE.datacenters_and_energy;
