...
function plotH100YearsTimeSeries(data) {
    if (!data.black_project_model) return;
...
function plotChipProductionReductionCcdf(data) {
    if (!data.black_project_model || !data.black_project_model.chip_production_reduction_ccdf) {
        document.getElementById('chipProductionReductionCcdfPlot').innerHTML = '<p>No chip production reduction data available</p>';
        return;
    }

    const reductionData = data.black_project_model.chip_production_reduction_ccdf;
...
function plotAiRdReductionCcdf(data) {
    if (!data.black_project_model || !data.black_project_model.ai_rd_reduction_ccdf) {
        document.getElementById('aiRdReductionCcdfPlot').innerHTML = '<p>No AI R&D reduction data available</p>';
        return;
    }

    const reductionData = data.black_project_model.ai_rd_reduction_ccdf;
    const primaryThreshold = DETECTION_CONFIG.PRIMARY_THRESHOLD;
...
function plotProjectH100YearsCcdf(data) {
    if (!data.black_project_model || !data.black_project_model.average_covert_compute_ccdf) {
    ...
    const likelihoodRatios = data.black_project_model.likelihood_ratios || DETECTION_CONFIG.LIKELIHOOD_RATIOS;
    ...
    for (const threshold of thresholdsReversed) {
        const ccdf = data.black_project_model.average_covert_compute_ccdf[threshold.value];
...
function plotTimeToDetectionCcdf(data) {
    if (!data.black_project_model || !data.black_project_model.time_to_detection_ccdf) {
    ...
    for (const threshold of thresholdsReversed) {
        const ccdf = data.black_project_model.time_to_detection_ccdf[threshold.value];
        if (ccdf && ccdf.length > 0) {
...
function updateDarkComputeModelDashboard(data) {
    ...
    const projectH100YearsValues = data.black_project_model?.individual_project_h100_years_before_detection || [];
    const projectTimeValues = data.black_project_model?.individual_project_time_before_detection || [];
