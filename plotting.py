"""
Plotting functions for model simulation results.

This module contains all plotting functionality for analyzing and visualizing
model simulation results. Each function accepts a Model object as input.
"""
...
def plot_probability_and_compute_over_time(model, save_path=None):
    """
    Create a dual y-axis plot showing US probability and H100e count over time.

    Args:
        model: Model object with simulation results
    ...
    simulations_to_plot = model.simulation_results[:100]
    ...
    ax1.set_ylabel('US Probability Covert Fab Exists', fontsize=12, fontweight='bold', color='darkblue')
    ...
    ax1.set_ylim([0, 1])
    ax1.set_xlim([model.year_us_prc_agreement_goes_into_force, model.end_year])
    ax1.grid(True, alpha=0.3, linestyle='--')

    # Create second y-axis for H100e count
    ax2 = ax1.twinx()
    ax2.set_ylabel('H100e in PRC Covert Project', fontsize=12, fontweight='bold', color='darkred')
    ...
    # Add info text
    info_text = f'Showing {len(simulations_to_plot)} simulations (of {len(model.simulation_results)} total)'
...
def plot_compute_before_detection(model, detection_threshold=0.5, save_path='compute_before_detection.png'):
    """
...
def plot_fab_characteristics(model, save_path: str = 'fab_characteristics.png'):
...
def plot_process_node_success_proportion(model, save_path: str = 'process_node_success.png'):
...
def display_aggregate_statistics(model):
    """
    ...
    Args:
        model: Model object with simulation results
    """
    plot_probability_and_compute_over_time(model, save_path='simulation_runs.png')
    plot_compute_before_detection(model, save_path='compute_before_detection.png')
