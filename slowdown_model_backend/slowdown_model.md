...
There are three AI projects that we model during the slowdown
- The largest U.S. project. 
- A covert PRC project. 
- The "proxy project" (A proxy of a potential covert PRC project operated by the United States.)

We also model a couple of counterfactual AI projects:
- The largest US company if there was no slowdown. 
- The largest PRC project if there was no slowdown. 

We also model "Negotiators" - governments or members of government that negotiate the terms of an AI agreement.
- The US government is a negotiator
- The Chinese government is also a negotiator

## Phases of the agreement

There are four main phases on the timeline:
1. Compute tracking
2. Compute reporting 
3. Cease training 
4. Cap and control
...
During compute tracking:
1.  for existing compute. By the end of the phase, both US and PRC negotiators obtain evidence of defection based on how much compute was reported.
2. Build inspector capacity for the next phase.

During the cease training phase, compute is either (1) shut down or (2) inference-only (there is no experiment compute during this phase).
- Each AI project class keeps track of how much compute the AI project has and the state of that compute.
- There is a parameter in Pa

During "Cap and elicit," enough compute is used for experiments to reach the capability cap. 
