...
const CustomHorizonChart = memo(({
  ...
  const xDomain = useMemo((): [number, number] => {
    ...
    // Use dynamic domain calculation that expands to show intersection
  ...
  // Add x property to chartData (already merged with trajectories in ProgressChartClient)
  const mergedData = useMemo<DataPoint[]>(() => {
    return chartData.map((point) => {
      return { ...point, x: point.year };
    });
  }, [chartData]);
    ...
    return mergedData
      ...
      .map(point => {
        ...
        trajectoryKeys.forEach(key => {
          ...
          if (typeof rawValue !== 'number' || !Number.isFinite(rawValue) || rawValue < yMin || rawValue > yMax) {
            sanitizedPoint[key] = null;
          }
        });

        const horizonValue = point.horizonLength;
        if (typeof horizonValue !== 'number' || !Number.isFinite(horizonValue) || horizonValue < yMin || horizonValue > yMax) {
      ...
      .filter(point => {
        ...
        return trajectoryKeys.some(key => {
          const value = point[key];
          return typeof value === 'number' && Number.isFinite(value);
        });
      });
  }, [mergedData, xDomain, horizonDomain, trajectoryKeys]);
  ...
  const customElements = useCallback((context: { chartWidth: number; chartHeight: number }) => {
    const { chartWidth, chartHeight } = context;
    const { xDomain: renderedXDomain, yDomain: renderedYDomain } = context as typeof context & {
      xDomain: [number, number];
      yDomain: [number, number];
    };
    const xScale = createScale({
      domain: renderedXDomain,
      range: [0, chartWidth],
      type: 'linear',
    });
    ...
    const handleMouseMove = (e: React.MouseEvent<SVGRectElement>) => {
      ...
      // Find if we're hovering over any benchmark (within 15px radius)
      let foundBenchmark: BenchmarkPoint | null = null;
      const hitRadius = 15;
      ...
      if (foundBenchmark) {
        ...
        setHoveredBenchmark(foundBenchmark);
      } else {
    ...
    return (
      <g>
        ...
        {visibleBenchmarks.map((benchmark, index) => {
          ...
          const cx = xScale(benchmark.year);
          const cy = yScale(benchmark.horizonLength);
          const ShapeComponent = shapeFunc(fill);
  ...
  }, [visibleBenchmarks, scHorizonMinutes, formatTimeDuration, automationCrossing]);
