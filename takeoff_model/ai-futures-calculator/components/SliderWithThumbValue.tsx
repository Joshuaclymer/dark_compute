...
export const SliderWithThumbValue: React.FC<SliderWithThumbValueProps> = ({
  ...
  const sliderMin = useStepCount ? 0 : (effectiveUseLogScale ? 0 : min);
  const sliderMax = useStepCount ? stepCount : (effectiveUseLogScale ? 100 : max);
  const sliderValue = (useStepCount || effectiveUseLogScale) ? valueToPosition(value) : value;
  ...
  const percentage = Number(rawPercentage.toFixed(6));

  const currentDisplayValue = formatValue ? formatValue(value) : displayValue;
  ...
  return (
    <div className="relative">
      ...
      />
      <div
        className="absolute top-full mt-1 text-xs text-primary font-medium transform -translate-x-1/2 pointer-events-none"
