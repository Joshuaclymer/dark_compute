...
def get_default_sampling_config():
    ...
    try:
        ...
        # Return both the config and the raw YAML text for editing
        return jsonify({"success": True, "config": data, "yaml_text": text, "source": "file"})
...
def run_monte_carlo():
    try:
        ...
        # Support two modes:
        # 1. Raw YAML text in "yaml_config" field (new, simple mode)
        # 2. Structured JSON config (legacy mode)
        yaml_text = payload.get("yaml_config")
        if yaml_text:
            # Parse YAML and use it as the effective config
            try:
                effective_cfg = yaml.safe_load(yaml_text)
                if not isinstance(effective_cfg, dict):
                    return jsonify({
                        "success": False,
                        "error": "YAML must parse to a dictionary/mapping"
                    }), 400
            ...
            effective_cfg: Dict[str, Any] = {
                ...
                "time_series_parameters": payload.get("time_series_parameters", {}),
        ...
        # Validate basic structure (only for legacy mode, YAML mode is more flexible)
        if not yaml_text:
            if not isinstance(effective_cfg.get("parameters", {}), dict):
...
def live_sc_histogram(job_id: str):
    """Serve a live-updating AC time histogram PNG based on current rollouts.jsonl."""
    if plt is None:
    ...
    try:
        ax.set_title("Live AC Time Distribution", fontsize=10)
        ...
        if total_n > 0:
            ...
            if num_no_sc > 0:
                ax.bar(nosc_x, num_no_sc, width=bin_width, edgecolor="black", alpha=0.6, label="No AC")
            ...
            ax.set_xlabel("AC Time (decimal year)")
            ax.set_ylabel("Count")
