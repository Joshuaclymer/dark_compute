...
_taste_distribution_cache = OrderedDict()
...
class TasteDistribution:
    ...
    The transformation function is: T = (A^ρ + (1 - A^ρ) * exp(x / (1 - A^ρ)))^(1/ρ)
    where x ~ Normal(μ, σ²), A is the taste_limit parameter, L is taste_limit_smoothing,
    and ρ = 2*ln(1/L - 1) / ln(A).
    ...
    def __init__(self,
        ...
        # Define the transformation function T = (A^ρ + (1 - A^ρ) * exp(x / (1 - A^ρ)))^(1/ρ)
        # where ρ = 2*ln(1/L - 1) / ln(A)
        A = self.taste_limit
        ...
        if cobb_douglas_limit:
            ...
            if c1 < 0:
                self._x_at_zero = -scale * np.log(A_pow_rho / scale)
            else:
                self._x_at_zero = float('-inf')
            ...
            def inverse_transform(T):
                ...
                if np.any(mid):
                    ...
                    if c1 > 0:
                        ...
                        ratio = (A_pow_rho - T_pow) / scale
                    ratio = np.clip(ratio, 1e-300, None)
                    result[mid] = -scale * np.log(ratio)
        ...
        def constraint_residuals(params):
            ...
            try:
                def integrand(x):
                    return self._transform(mu_trial + sigma_trial * x) * norm.pdf(x)
                mean_val, _ = quad(integrand, -np.inf, np.inf, limit=100)
        ...
        # Precompute the actual mean for get_mean()
        try:
            def integrand(x):
                return self._transform(self.mu + self.sigma * x) * norm.pdf(x)
            self._computed_mean, _ = quad(integrand, -np.inf, np.inf, limit=100)
        except:
    ...
    def get_mean_with_floor(self, floor_taste: float) -> float:
        ...
        if floor_taste < self._floor_grid_low:
            return self._computed_mean

        # Fast path: floor above grid range (mean ≈ floor)
        if floor_taste > self._floor_grid_high:
            return floor_taste

        # Fast path: floor at or above taste_limit
        if floor_taste >= self.taste_limit:
            return floor_taste

        # Interpolate using precomputed grid
        # Use numpy's interp for fast 1D interpolation
        result = np.interp(floor_taste, self._floor_grid_floors, self._floor_grid_means)

        # Ensure result is at least the floor (numerical precision safeguard)
        return max(result, floor_taste)
...
class AutomationModel:
    ...
    def _precompute_frontier(self, params) -> Optional[_FrontierPrecomp]:
        try:
            ...
            log_B = np.full_like(log_Eaut, -np.inf)
            # B[0] = 0 => log_B[0] = -inf by convention

            for k in range(M - 1):
                log_inc = log_di + log_half + np.logaddexp(log_w[k], log_w[k+1])
                # B[k+1] = B[k] + inc  in log-space:
                log_B[k+1] = np.logaddexp(log_B[k], log_inc)
    ...
    def coding_labor_optimal_ces(self, H: float, C: float, logE: float, params, return_details: bool = False):
        ...
        try:
            ...
            L = float(np.exp(log_L))
            # if log_L > 40:
            #     logger.warning(f"log_L is: {log_L}")
            if return_details:
                human = float(np.exp(log_human))
                comp = float(np.exp(log_comp))
...
class Parameters:
    ...
    asi_above_siar_vs_tedai_above_sar_difficulty: float = field(default_factory=lambda: cfg.DEFAULT_PARAMETERS['asi_above_siar_vs_tedai_above_sar_difficulty'])
    ...
    weight_stealing_mode: bool = field(default_factory=lambda: cfg.DEFAULT_PARAMETERS['weight_stealing_mode'])
    # List of times or milestone names when weights are stolen from the leading project
    # Can contain floats (decimal years, e.g., 2030.5) or strings (milestone names, e.g., "SC", "AC", "ASI")
    years_weights_are_stolen: Optional[List[Union[float, str]]] = field(default_factory=lambda: cfg.DEFAULT_PARAMETERS['years_weights_are_stolen'])
    # Minimum model progress - when weights are stolen, the AI model capability is at least this level
    # This is similar to blacksite_initial_model_progress but can be updated dynamically
    stolen_model_progress: Optional[float] = field(default_factory=lambda: None)
    # Algorithm stealing: time or milestone up to which the stealing project has the same software efficiency
    # as the leading project. Can be a float (decimal year) or string (milestone name like "SC", "AC", "ASI")
    stealing_algorithms_up_to: Optional[Union[float, str]] = field(default_factory=lambda: cfg.DEFAULT_PARAMETERS.get('stealing_algorithms_up_to', None))
...
def compute_software_progress_rate(research_stock: float, research_effort: float, 
    ...
    """
    # Input validation
    if not all(np.isfinite([research_stock, research_effort, r_software])):
...
def _compute_ai_research_taste_sd_per_progress(cumulative_progress: float, params: Parameters) -> float:
    ...
    """
    # Use taste distribution from params
    try:
        taste_distribution = params.taste_distribution
    except Exception as e:
        logger.warning(f"Error accessing TasteDistribution: {e}")
...
def compute_aggregate_research_taste(ai_research_taste: float,
                                   taste_distribution: TasteDistribution) -> float:
    """
...
def progress_rate_at_time(t: float, state: List[float], time_series_data: TimeSeriesData, params: Parameters) -> List[float]:
    ...
    # Weight stealing: use the stolen model's progress level if it's higher
    if params.weight_stealing_mode and params.stolen_model_progress is not None:
        model_progress = max(cumulative_progress, params.stolen_model_progress)
    if not np.isfinite(cumulative_progress):
    ...
    try:
        ...
        # performance note: this is redundant in the loop, also could be vectorized probably.
...
def integrate_progress(time_range: List[float], initial_progress: float, initial_research_stock: float, time_series_data: TimeSeriesData, 
...
class ProgressModel:
        ...
        try:
            if self.params.horizon_extrapolation_type == "exponential":
                ...
                try:
                    ...
                    _include_gap_flag = False
                target_horizon = self.params.pre_gap_ac_time_horizon if _include_gap_flag else self.params.ac_time_horizon_minutes
                if target_horizon > 0:
                    try:
                        ...
                        if _include_gap_flag:
                            ...
                            try:
                                ...
                                year_label = 'anchor'
                            logger.info(
                                f"Gap-included mode: using pre-gap SC horizon {self.params.pre_gap_ac_time_horizon} and "
                    ...
                    if actual_horizon is not None and actual_horizon > 0:
                        ...
                        try:
                            ...
                            if np.isfinite(predicted_horizon) and predicted_horizon > 0:
                                ...
                                logger.debug(f"Invalid predicted horizon for {model_name}/{agent_name}: {predicted_horizon}")
                        except Exception as e:
                            logger.debug(f"Error predicting horizon for {model_name}/{agent_name}: {e}")
                            continue
    ...
    def compute_human_only_trajectory(self, time_range: List[float], initial_progress: Optional[float] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        times, progress_values, research_stock_values = integrate_progress(time_range, initial_progress, initial_research_stock_val, self.data, human_only_params)
        progress_rates = []
        research_efforts = []
        sw_progress_rates = []
        ...
        for i, (t, p, rs) in enumerate(zip(times, progress_values, research_stock_values)):
            ...
            sw_progress_rates.append(compute_software_progress_rate(rs, research_effort, human_only_params.r_software))
    ...
    def compute_progress_trajectory(self, time_range: List[float], initial_progress: Optional[float] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Option B approach: Modulate compute directly to keep speedup below cap.
        #
        # Key insight: Speedup is computed from progress level using present-day resources.
        # So speedup = f(progress), where f is monotonically increasing.
        # To cap speedup, we need to cap progress. To cap progress, we need to reduce compute.
        #
        # IMPORTANT: Simple post-hoc compute estimation doesn't work because when you re-run
        # the ODE from scratch with estimated compute, you get a DIFFERENT trajectory.
        # The only correct approach is iterative: find compute that produces capped trajectory.
        #
        # Strategy:
        # 1. Find the target capped progress (where speedup = cap)
        # 2. Use binary search on global compute scale to find scale that produces target speedup
        # 3. Re-run ODE with scaled compute until we converge on a trajectory that stays at/below cap
        speedup_cap_at_times = None
        capped_compute_at_times = None  # Will hold the compute that enforces the cap
        if self.data.capability_cap is not None:
            ...
            # Apply small margin below the cap to account for numerical precision
            speedup_cap_at_times = speedup_cap_at_times * 0.98
            logger.info(f"Applying AI R&D speedup cap via compute modulation (range: {np.min(speedup_cap_at_times):.1f} to {np.max(speedup_cap_at_times):.1f})")

            # Get full compute at each time for reference
            full_inference_compute_at_times = np.array([_log_interp(t, self.data.time, self.data.inference_compute) for t in times])
            full_experiment_compute_at_times = np.array([_log_interp(t, self.data.time, self.data.experiment_compute) for t in times])
            full_compute_at_times = full_inference_compute_at_times + full_experiment_compute_at_times

            # Helper function to compute speedup at a given progress level using present-day resources
            def compute_speedup_at_progress(prog):
                """Compute AI R&D speedup at given progress using present-day resources."""
                if getattr(self.params, 'coding_labor_mode', 'simple_ces') == 'optimal_ces':
                    logE = float(np.log(cfg.BASE_FOR_SOFTWARE_LOM) * prog)
                    L_opt_present = self.params.automation_model.coding_labor_optimal_ces(
                    ...
                    automation_frac = self.params.automation_model.get_automation_fraction(prog)
                    coding_labor_present = compute_coding_labor(
                        automation_frac, present_day_inference_compute, present_day_human_labor,
                ...
                ai_research_taste = compute_ai_research_taste(prog, self.params)
                ...
                software_rate_present = compute_software_progress_rate(
                    present_day_research_stock, research_effort_present, self.params.r_software
                )
                return software_rate_present / present_day_sw_progress_rate if present_day_sw_progress_rate > 0 else 0.0

            # Save original uncapped progress values
            original_progress_values = progress_values.copy()
            original_research_stock = research_stock_values.copy()

            # Step 1: Find the capped progress trajectory (where speedup stays at cap)
            ...
            for i in range(1, len(times)):
                ...
                uncapped_speedup = compute_speedup_at_progress(candidate_progress)

                if uncapped_speedup <= cap_speedup:
                    ...
                    for _ in range(20):
                        ...
                        mid_speedup = compute_speedup_at_progress(mid_prog)
            ...
            # Step 2: Estimate effective compute using simple ratio (fast approximation)
            # Instead of iterative ODE re-runs (slow), we estimate compute by scaling proportionally:
            # When uncapped_speedup > cap, effective_compute = full_compute * (cap / uncapped_speedup)
            # This is an approximation but much faster and sufficient for visualization purposes.
            logger.info("Estimating effective compute via simple ratio (fast mode)...")

            capped_compute_at_times = np.zeros(len(times))
            for i in range(len(times)):
                uncapped_speedup = compute_speedup_at_progress(original_progress_values[i])
                cap_speedup = speedup_cap_at_times[i]

                if uncapped_speedup <= cap_speedup or cap_speedup == float('inf'):
                    # Below cap - use full compute
                    capped_compute_at_times[i] = full_compute_at_times[i]
                else:
                    # Above cap - scale down proportionally
                    # This approximation assumes roughly linear relationship in log-space
                    scale_factor = cap_speedup / uncapped_speedup if uncapped_speedup > 0 else 1.0
                    capped_compute_at_times[i] = full_compute_at_times[i] * scale_factor

            logger.info(f"Computed capped compute range: [{np.min(capped_compute_at_times):.2e}, {np.max(capped_compute_at_times):.2e}]")

            progress_values = capped_progress_values
            logger.info(f"Applied capability cap via compute modulation: progress range [{progress_values[0]:.3f}, {progress_values[-1]:.3f}]")
            logger.info(f"Computed capped compute range: [{np.min(capped_compute_at_times):.2e}, {np.max(capped_compute_at_times):.2e}]")
        ...
        # Profiling accumulators for detailed timing
        _profile_times = {
            'progress_rate': 0.0,
            'input_interp': 0.0,
            'automation': 0.0,
            'taste': 0.0,
            'taste_compute_ai': 0.0,
            'taste_get_sd': 0.0,
            'taste_get_quantile': 0.0,
            'taste_aggregate': 0.0,
            'coding_labor': 0.0,
            'exp_capacity': 0.0,
            'research_effort': 0.0,
        ...
        for i, (t, progress, rs) in enumerate(zip(times, progress_values, research_stock_values)):
            try:
                ...
                _profile_times['progress_rate'] += time.perf_counter() - _t0
                ...
                _profile_times['input_interp'] += time.perf_counter() - _t0
                ...
                _profile_times['automation'] += time.perf_counter() - _t0
                ...
                _profile_times['taste'] += time.perf_counter() - _t_taste_start
                ...
                _profile_times['coding_labor'] += time.perf_counter() - _t0
                ...
                if self.params.rho_experiment_capacity < -1e-4:
                ...
                _profile_times['exp_capacity'] += time.perf_counter() - _t0
                ...
                _profile_times['research_effort'] += time.perf_counter() - _t0
                ...
                _profile_times['software_rate'] += time.perf_counter() - _t0
                ...
                software_efficiency_val = progress - initial_progress - training_compute_val
                ...
                _profile_times['training_compute'] += time.perf_counter() - _t0

                # CAPABILITY (ECI)
                _t0 = time.perf_counter()
                try:
                    capability_eci = self.get_capability(training_compute_val, software_efficiency_val)
                except:
                    logger.warning(f"Couldn't compute ECI at time {t}.")
                    capability_eci = 0
                capability_ecis.append(capability_eci)
                _profile_times['capability'] += time.perf_counter() - _t0
                ...
                _profile_times['horizon'] += time.perf_counter() - _t0
                ...
                _profile_times['human_only'] += time.perf_counter() - _t0
                ...
                _profile_times['labor_contrib'] += time.perf_counter() - _t0
        ...
        logger.info("=== Metrics Loop Profiling Breakdown ===")
        _total_profiled = sum(_profile_times.values())

        # Group taste subcategories
        taste_subcategories = ['taste_compute_ai', 'taste_get_sd', 'taste_get_quantile', 'taste_aggregate']
        taste_details = {k: _profile_times[k] for k in taste_subcategories if k in _profile_times}

        for category, t in sorted(_profile_times.items(), key=lambda x: -x[1]):
            # Skip taste subcategories in main list (we'll show them separately)
            if category in taste_subcategories:
                continue
            _pct = 100.0 * t / _dt_metrics_loop if _dt_metrics_loop > 0 else 0.0
            _avg_ms = 1000.0 * t / _num_iters if _num_iters > 0 else 0.0
            logger.info(f"  {category:20s}: {t:7.3f}s ({_pct:5.1f}%) avg {_avg_ms:6.3f} ms/iter")

            # Show taste breakdown after the main taste entry
            if category == 'taste' and taste_details:
                for subcat, subt in sorted(taste_details.items(), key=lambda x: -x[1]):
                    _subpct = 100.0 * subt / _dt_metrics_loop if _dt_metrics_loop > 0 else 0.0
                    _subavg_ms = 1000.0 * subt / _num_iters if _num_iters > 0 else 0.0
                    logger.info(f"    ↳ {subcat:18s}: {subt:7.3f}s ({_subpct:5.1f}%) avg {_subavg_ms:6.3f} ms/iter")

        _unaccounted = _dt_metrics_loop - _total_profiled
        _unaccounted_pct = 100.0 * _unaccounted / _dt_metrics_loop if _dt_metrics_loop > 0 else 0.0
        logger.info(f"  {'unaccounted':20s}: {_unaccounted:7.3f}s ({_unaccounted_pct:5.1f}%)")
        logger.info("=" * 45)
        ...
        # Add capped compute values if capability cap was applied
        if capped_compute_at_times is not None:
            self.results['capped_compute'] = capped_compute_at_times
    ...
    def compute_milestones(self):
        ...
        milestones = {
            ...
            'TED-AI': {
                'metric': 'ai_research_taste_sd',
                'target': cfg.TOP_RESEARCHER_SD * (1.0 + self.params.ted_ai_m2b),  # Linearly spaced in SD
                'interpolation_type': 'exponential'
            },
            'ASI': {
                'metric': 'ai_research_taste_sd',
                'target': cfg.TOP_RESEARCHER_SD * (1.0 + 2.0 + self.params.asi_above_siar_vs_tedai_above_sar_difficulty * self.params.ted_ai_m2b),  # Linearly spaced in SD
                'interpolation_type': 'exponential'
            },
        ...
        for i in range(len(milestones_list) - 1):
            ...
            if self.results['takeoff_progress_multipliers'] and 'takeoff_start_stats' in self.human_only_results:
...
class BlacksiteProgressModel(ProgressModel):
    """
    Progress model for blacksite mode.
    """
    def __init__(self, params: Parameters, data: TimeSeriesData, main_model: ProgressModel):
...
class WeightStealingProgressModel(ProgressModel):
    """
    Progress model that simulates weight stealing and/or algorithm stealing from a leading project.

    This model tracks two separate progress trajectories:
    1. The "leading project" - a more capable project with more resources
    2. The "stealing project" - a project that can steal from the leading project

    Two types of stealing are supported:

    Weight Stealing (years_weights_are_stolen):
        At specified times/milestones, the stealing project gains access to the leading
        project's MODEL CAPABILITIES (automation fraction, AI research taste, etc.) but
        does NOT inherit their progress or research stock. The stealing project continues
        to accumulate progress using their own resources, but with the enhanced AI
        capabilities from the stolen model.

        This is realistic because:
        - Stealing weights gives you a more capable AI assistant
        - But you still need to run experiments with your own compute
        - And you don't get the leading project's accumulated research knowledge

    Algorithm Stealing (stealing_algorithms_up_to):
        Up to a specified time/milestone, the stealing project has the same SOFTWARE
        EFFICIENCY as the leading project. This models scenarios where algorithms,
        techniques, and research findings are stolen, giving the stealing project the
        benefit of the leading project's software improvements.

        After the cutoff time, the stealing project continues with its own trajectory
        but keeps the software efficiency gains accumulated up to that point.

    Both types of stealing can be combined in a single simulation.
    """

    def __init__(self, params: Parameters, stealing_project_data: TimeSeriesData,
                 leading_project_data: TimeSeriesData):
        """
        Initialize the weight stealing progress model.

        Args:
            params: Model parameters
            stealing_project_data: Time series data for the stealing project (lower resources)
            leading_project_data: Time series data for the leading project (higher resources)
        """
        # Initialize as a standard ProgressModel using stealing project data
        super().__init__(params, stealing_project_data)

        # Store leading project data
        self.leading_project_data = leading_project_data
        self.stealing_project_data = stealing_project_data

        # Results for leading project will be stored separately
        self.leading_project_results = {}

    def _resolve_milestone_to_time(self, milestone_name: str, milestones: Dict) -> Optional[float]:
        """
        Resolve a milestone name to its time value.

        Args:
            milestone_name: Name of the milestone (e.g., "AC", "SC", "AI2027-SC", "ASI")
            milestones: Dictionary of milestones from the model results

        Returns:
            The time when the milestone is reached, or None if not found/not reached
        """
        # Common aliases for milestone names
        aliases = {
            'SC': 'AI2027-SC',
        ...
        if milestone_name in milestones:
            time_val = milestones[milestone_name].get('time')
            if time_val is not None and time_val != 'N/A':
                return float(time_val)

        # Try alias
        if milestone_name in aliases:
            alias = aliases[milestone_name]
            if alias in milestones:
                time_val = milestones[alias].get('time')
                if time_val is not None and time_val != 'N/A':
                    return float(time_val)
    ...
    def compute_progress_trajectory(self, time_range: List[float], initial_progress: Optional[float] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Compute progress trajectory with weight stealing events.

        The algorithm:
        1. First, compute the complete leading project trajectory
        2. Resolve any milestone names in stealing times to actual times
        3. Run a calibration model for the stealing project to get properly calibrated parameters
        4. Use custom ODE integration that dynamically updates stolen_model_progress based on
           the most recent stealing event
        ...
        Note:
            years_weights_are_stolen can contain either:
            - Numeric values (decimal years, e.g., 2030.5)
            - Milestone names as strings (e.g., "AC", "SC", "AI2027-SC", "ASI")
            Milestone names will be resolved to times from the leading project's trajectory.
        """
        if initial_progress is None:
            initial_progress = 0.0

        # Step 1: Compute the leading project trajectory first
        # We need this before resolving milestone names
        leading_params = copy.deepcopy(self.params)
        leading_params.weight_stealing_mode = False
        leading_params.years_weights_are_stolen = None
        leading_params.stolen_model_progress = None

        leading_model = ProgressModel(leading_params, self.leading_project_data)
        leading_model.compute_progress_trajectory(time_range, initial_progress)
        self.leading_project_results = leading_model.results

        # Store leading project model for reference
        self.leading_project_model = leading_model

        # Step 2: Resolve milestone names to times
        raw_stealing_times = self.params.years_weights_are_stolen or []
        stealing_times = []
        leading_milestones = self.leading_project_results.get('milestones', {})

        for item in raw_stealing_times:
            if isinstance(item, str):
                # It's a milestone name - resolve it
                resolved_time = self._resolve_milestone_to_time(item, leading_milestones)
                if resolved_time is not None:
                    logger.info(f"Resolved milestone '{item}' to time {resolved_time:.3f}")
                    stealing_times.append(resolved_time)
                else:
                    logger.warning(f"Could not resolve milestone '{item}' - milestone not reached or not found")
            else:
                # It's a numeric time
                stealing_times.append(float(item))

        # Sort and filter stealing times to be within time_range
        stealing_times = sorted([t for t in stealing_times if time_range[0] <= t <= time_range[1]])

        logger.info(f"Weight stealing mode: {len(stealing_times)} stealing events at times {stealing_times}")

        # Step 2b: Resolve algorithm stealing cutoff time
        algorithm_stealing_cutoff = None
        if self.params.stealing_algorithms_up_to is not None:
            if isinstance(self.params.stealing_algorithms_up_to, str):
                # It's a milestone name - resolve it
                algorithm_stealing_cutoff = self._resolve_milestone_to_time(
                    self.params.stealing_algorithms_up_to, leading_milestones)
                if algorithm_stealing_cutoff is not None:
                    logger.info(f"Resolved algorithm stealing milestone '{self.params.stealing_algorithms_up_to}' to time {algorithm_stealing_cutoff:.3f}")
                else:
                    logger.warning(f"Could not resolve algorithm stealing milestone '{self.params.stealing_algorithms_up_to}' - milestone not reached or not found")
            else:
                algorithm_stealing_cutoff = float(self.params.stealing_algorithms_up_to)
                logger.info(f"Algorithm stealing cutoff time: {algorithm_stealing_cutoff:.3f}")

        # Step 3: If no stealing events and no algorithm stealing, just compute normal trajectory
        if not stealing_times and algorithm_stealing_cutoff is None:
            self.params.weight_stealing_mode = False
            return super().compute_progress_trajectory(time_range, initial_progress)

        # Step 4: Run a calibration model for the stealing project
        # This ensures we have valid human_only_results and calibrated parameters
        calibration_params = copy.deepcopy(self.params)
        calibration_params.weight_stealing_mode = False
        calibration_params.years_weights_are_stolen = None
        calibration_params.stolen_model_progress = None
        calibration_model = ProgressModel(calibration_params, self.stealing_project_data)
        calibration_model.compute_progress_trajectory(time_range, initial_progress)

        # Store calibrated parameters and results for metric computation
        self.human_only_results = calibration_model.human_only_results
        self.horizon_trajectory = calibration_model.horizon_trajectory
        self.taste_distribution = calibration_model.taste_distribution

        # Copy calibrated params for the stealing project trajectory
        self.params = copy.deepcopy(calibration_model.params)

        # Step 5: Create interpolators for leading project data
        leading_times = np.array(self.leading_project_results['times'])
        leading_progress = np.array(self.leading_project_results['progress'])
        leading_software_efficiency = np.array(self.leading_project_results['software_efficiency'])

        # Also get the calibration model's software efficiency for comparison
        calibration_times = np.array(calibration_model.results['times'])
        calibration_software_efficiency = np.array(calibration_model.results['software_efficiency'])

        def get_stolen_model_progress_at_time(t: float) -> Optional[float]:
            """Get the stolen model progress at time t, based on most recent stealing event."""
            # Find the most recent stealing event before or at time t
            relevant_steal_times = [st for st in stealing_times if st <= t]
            if not relevant_steal_times:
                return None
            # Get the most recent stealing time
            most_recent_steal = max(relevant_steal_times)
            # Return the leading project's progress at that steal time
            return float(np.interp(most_recent_steal, leading_times, leading_progress))

        # Create a single params copy for the ODE integration - avoids expensive deep copy on every call
        ode_params = copy.deepcopy(self.params)

        def progress_rate_with_dynamic_stealing(t, state, data, params_unused):
            """Custom progress rate function that dynamically computes stolen_model_progress."""
            # Update the pre-copied params with correct stolen_model_progress for time t
            # (params_unused is ignored - we use ode_params from closure to avoid deep copy)
            stolen_progress = get_stolen_model_progress_at_time(t)
            if stolen_progress is not None:
                ode_params.weight_stealing_mode = True
                ode_params.stolen_model_progress = stolen_progress
            else:
                ode_params.weight_stealing_mode = False
                ode_params.stolen_model_progress = None
            return progress_rate_at_time(t, state, data, ode_params)

        # Step 6: Integrate using the custom progress rate function
        # Get initial research stock from the calibration model
        initial_research_stock = calibration_model.results['research_stock'][0]

        # Generate time points
        num_points = len(calibration_model.results['times'])
        ...
        def ode_func(state, t):
            return progress_rate_with_dynamic_stealing(t, state, self.data, self.params)

        initial_state = [initial_progress, initial_research_stock]
        solution = odeint(ode_func, initial_state, times)

        progress_values = solution[:, 0]
        research_stock_values = solution[:, 1]

        # Step 7: Apply algorithm stealing boost if enabled
        # Algorithm stealing means the stealing project has the same software efficiency
        # as the leading project up to the cutoff time
        if algorithm_stealing_cutoff is not None:
            # Compute the software efficiency boost at each time point
            for i, t in enumerate(times):
                if t <= algorithm_stealing_cutoff:
                    # Get leading project's software efficiency at this time
                    leading_sw_eff = float(np.interp(t, leading_times, leading_software_efficiency))
                    # Get what the stealing project's software efficiency would be without stealing
                    # (from the calibration model)
                    calibration_sw_eff = float(np.interp(t, calibration_times, calibration_software_efficiency))
                    # The boost is the difference
                    sw_eff_boost = leading_sw_eff - calibration_sw_eff
                    if sw_eff_boost > 0:
                        progress_values[i] += sw_eff_boost

            logger.info(f"Algorithm stealing applied up to t={algorithm_stealing_cutoff:.3f}")
            # Log the boost at the cutoff time
            cutoff_idx = np.searchsorted(times, algorithm_stealing_cutoff)
            if cutoff_idx < len(times):
                leading_sw_eff_at_cutoff = float(np.interp(algorithm_stealing_cutoff, leading_times, leading_software_efficiency))
                calibration_sw_eff_at_cutoff = float(np.interp(algorithm_stealing_cutoff, calibration_times, calibration_software_efficiency))
                logger.info(f"  At cutoff: leading SW efficiency = {leading_sw_eff_at_cutoff:.3f}, "
                           f"calibration SW efficiency = {calibration_sw_eff_at_cutoff:.3f}, "
                           f"boost = {leading_sw_eff_at_cutoff - calibration_sw_eff_at_cutoff:.3f}")

        # Log weight stealing events
        for steal_time in stealing_times:
            idx = np.searchsorted(times, steal_time)
            if idx < len(times):
                own_progress_at_steal = progress_values[idx]
                stolen_progress_at_steal = get_stolen_model_progress_at_time(steal_time)
                logger.info(f"Weight stealing at t={steal_time:.3f}: "
                           f"stealing project progress {own_progress_at_steal:.3f}, "
                           f"stealing model with progress level {stolen_progress_at_steal:.3f} "
                           f"(gaining AI capabilities but keeping own progress)")

        # Store the final params with the last stolen_model_progress for metric computation
        final_stolen_progress = get_stolen_model_progress_at_time(time_range[1])
        self.params.weight_stealing_mode = True
        self.params.stolen_model_progress = final_stolen_progress

        # Now compute all the derived metrics
        self._compute_comprehensive_metrics(times, progress_values, research_stock_values,
                                            initial_progress, stealing_times)

        return times, progress_values, research_stock_values

    def _compute_comprehensive_metrics(self, times: np.ndarray, progress_values: np.ndarray,
                                        research_stock_values: np.ndarray, initial_progress: float,
                                        stealing_times: List[float]):
        """
        Compute all derived metrics after the progress trajectory has been computed.

        This computes metrics while properly accounting for stolen model capabilities
        at different points in time.
        """
        # Get present day stats from human_only_results
        present_day = self.params.present_day
        present_day_human_labor = self.human_only_results['anchor_stats']['human_labor']
        present_day_inference_compute = self.human_only_results['anchor_stats']['inference_compute']
        present_day_experiment_compute = self.human_only_results['anchor_stats']['experiment_compute']
        ...
        # Sort stealing times for lookup
        stealing_times_sorted = sorted(stealing_times)

        def get_model_progress_at_time(t: float, own_progress: float) -> float:
            """Get the effective model progress at time t, accounting for stolen weights."""
            # Find the most recent stealing event before or at time t
            stolen_progress = None
            for steal_time in stealing_times_sorted:
                if steal_time <= t:
                    # Get leading project progress at this steal time
                    stolen_progress = np.interp(
                        steal_time,
                        self.leading_project_results['times'],
                        self.leading_project_results['progress']
                    )
                else:
                    break
            # Model progress is the max of own progress and stolen model progress
            if stolen_progress is not None:
                return max(own_progress, stolen_progress)
            return own_progress
        ...
        training_compute = []
        experiment_capacity = []
        model_progress_values = []  # Track model progress separately

        # Create a single params copy for the loop - only modify weight_stealing fields
        # This avoids expensive deep copy on every iteration
        loop_params = copy.deepcopy(self.params)

        for i, (t, progress, rs) in enumerate(zip(times, progress_values, research_stock_values)):
            try:
                # Get effective model progress (accounting for stolen weights)
                model_progress = get_model_progress_at_time(t, progress)
                model_progress_values.append(model_progress)

                # Update params with correct stolen_model_progress for this time point
                if model_progress > progress:
                    loop_params.weight_stealing_mode = True
                    loop_params.stolen_model_progress = model_progress
                else:
                    loop_params.weight_stealing_mode = False
                    loop_params.stolen_model_progress = None

                state = [progress, rs]
                rates = progress_rate_at_time(t, state, self.data, loop_params)
                progress_rates.append(rates[0])
                research_efforts.append(rates[1])
                ...
                # Automation fraction - use MODEL progress (which may be from stolen weights)
                automation_fraction = compute_automation_fraction(model_progress, self.params)
                automation_fractions.append(automation_fraction)

                # Research taste - use MODEL progress
                ai_research_taste = compute_ai_research_taste(model_progress, self.params)
                ai_research_taste_sd = self.taste_distribution.get_sd_of_taste(ai_research_taste)
                ai_research_taste_quantile = self.taste_distribution.get_quantile_of_taste(ai_research_taste)
                aggregate_research_taste = compute_aggregate_research_taste(ai_research_taste, self.params.taste_distribution)
                ...
                # Coding labor - use MODEL progress for capability
                if getattr(self.params, 'coding_labor_mode', 'simple_ces') == 'optimal_ces':
                    H = float(L_HUMAN)
                    C = float(inference_compute)
                    logE = float(np.log(cfg.BASE_FOR_SOFTWARE_LOM) * model_progress)  # Use model_progress!
                    automation_model = self.params.automation_model
                    L_opt = automation_model.coding_labor_optimal_ces(H, C, logE, self.params)
                    L_opt_present_resources = automation_model.coding_labor_optimal_ces(
                        present_day_human_labor, present_day_inference_compute, logE, self.params)

                    coding_labor = L_opt if L_opt is not None else H
                    coding_labor_with_present_resources = L_opt_present_resources if L_opt_present_resources is not None else present_day_human_labor
                    ...
                    coding_labor = L_HUMAN
                    serial_coding_labor = compute_coding_labor(
                        automation_fraction, inference_compute, L_HUMAN,
                        self.params.rho_coding_labor, self.params.parallel_penalty, self.params.coding_labor_normalization
                    )
                    coding_labor_with_present_resources = present_day_human_labor
                    serial_coding_labor_with_present_resources = compute_coding_labor(
                ...
                # Research effort with present resources
                research_effort_present_resources = compute_research_effort(
                    present_day_experiment_compute, serial_coding_labor_with_present_resources,
                    self.params.alpha_experiment_capacity, self.params.rho_experiment_capacity,
                    self.params.experiment_compute_exponent, aggregate_research_taste
                )

                # Software progress rate
                software_rate = compute_software_progress_rate(rs, current_research_effort, self.params.r_software)
                software_progress_rates.append(software_rate if np.isfinite(software_rate) else 0.0)
                ...
                # Horizon length - use MODEL progress
                horizon_length = 0.0
                if self.horizon_trajectory is not None:
                    try:
                        horizon_length = self.horizon_trajectory(model_progress)
                ...
                human_only_coding_labor = L_HUMAN
                human_only_serial_coding_labor = L_HUMAN ** self.params.parallel_penalty
                human_only_aggregate_research_taste = compute_aggregate_research_taste(0, self.params.taste_distribution)
                human_only_research_effort = compute_research_effort(
                    experiment_compute, human_only_serial_coding_labor,
                    self.params.alpha_experiment_capacity, self.params.rho_experiment_capacity,
                    self.params.experiment_compute_exponent, human_only_aggregate_research_taste
                )
                human_only_software_rate = compute_software_progress_rate(rs, human_only_research_effort, self.params.r_software)
                human_only_overall_rate = compute_overall_progress_rate(human_only_software_rate, training_compute_growth_rate)
                human_only_progress_rates.append(human_only_overall_rate if np.isfinite(human_only_overall_rate) else 0.0)

                # Labor contributions
                human_contrib = L_HUMAN
                ai_contrib = max(0.0, coding_labor - human_contrib)
                human_labor_contributions.append(human_contrib)
                ai_labor_contributions.append(ai_contrib)
                ...
                if self.params.parallel_penalty and self.params.parallel_penalty != 0:
                    ...
                    ai_coding_labor_multipliers.append(0.0)
                    ai_coding_labor_mult_ref_present_day.append(0.0)

                ai_sw_progress_mult_ref_present_day.append(software_rate_present_resources / present_day_sw_progress_rate if present_day_sw_progress_rate > 0 else 0.0)
                takeoff_progress_multipliers.append(0.0)  # Simplified for now

            except Exception as e:
                logger.warning(f"Error computing metrics at t={t}: {e}")
                ...
                ai_research_taste_sds.append(0.0)
                ai_research_taste_quantiles.append(0.0)
                aggregate_research_tastes.append(1.0)
                coding_labors.append(0.0)
                coding_labors_with_present_resources.append(0.0)
                serial_coding_labors.append(0.0)
                software_progress_rates.append(0.0)
                software_progress_rates_present_resources.append(0.0)
                software_efficiency.append(0.0)
                human_only_progress_rates.append(0.0)
                ai_labor_contributions.append(0.0)
                human_labor_contributions.append(0.0)
                ai_coding_labor_multipliers.append(0.0)
                ai_coding_labor_mult_ref_present_day.append(0.0)
                ai_sw_progress_mult_ref_present_day.append(0.0)
                takeoff_progress_multipliers.append(0.0)
                discounted_exp_compute.append(0.0)
                horizon_lengths.append(0.0)
                effective_compute.append(0.0)
                training_compute.append(0.0)
                experiment_capacity.append(0.0)
                model_progress_values.append(progress)

        # Compute AA time (when progress reaches progress_at_aa)
        aa_time = None
        if self.params.progress_at_aa is not None:
            sc_progress_target = self.params.progress_at_aa
            if progress_values[-1] >= sc_progress_target:
        ...
        self.results = {
            'times': times,
            'progress': progress_values,
            'model_progress': model_progress_values,  # Track model progress separately
            'research_stock': research_stock_values,
            'automation_fraction': automation_fractions,
            'ai_research_taste': ai_research_tastes,
            'ai_research_taste_sd': ai_research_taste_sds,
            ...
            'experiment_capacity': experiment_capacity,
            'aa_time': aa_time,
            'sc_progress_level': self.params.progress_at_aa,
            'ai2027_sc_time': ai2027_sc_time,
            'input_time_series': {
                'time': self.data.time,
                'L_HUMAN': self.data.L_HUMAN,
                'inference_compute': self.data.inference_compute,
            ...
            'r_software': self.params.r_software,
            # Weight stealing specific results
            'weight_stealing_mode': True,
            'years_weights_stolen': self.params.years_weights_are_stolen,
            'leading_project_results': self.leading_project_results,
        }
