...
("gap" vs "no gap"), and saves two histograms of AC times to the run dir:
...
def main() -> None:
    ...
    print(f"Loaded {total_gap} records for include_gap=gap: {len(aa_gap)} finite AA times, +{no_sc_gap} No AC")
    print(f"Loaded {total_nogap} records for include_gap=no gap: {len(aa_nogap)} finite AA times, +{no_sc_nogap} No AC")
